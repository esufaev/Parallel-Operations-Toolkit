...
TEST_CASE("pot::executors::bench_thread_pool_executor")
{
    for (int64_t&& vec_size : { 1e2, 1e3, 1e4, 1e6 })
    {
        ...
        printf("%*s\t", column_width, "local (us)");
        printf("%*s\n", column_width, "global (us)");

        for(int64_t&& thread_count : {1, 2, 4, 8, 12, 16, 24, 32, 64, 128, 256, 512})
        {
            constexpr int64_t test_count = 100;
            const auto [time_local, time_global] = bench_thread_pool_lg<std::chrono::microseconds>(test_count, thread_count, vec_size);
