...
namespace pot::simd
{
    ...
    class simd_auto
    {
        static constexpr size_t align_bound =
            (alignof(scalar_type) > alignof(std::max_align_t) ? alignof(scalar_type) : alignof(std::max_align_t));

        using vector_type = scalar_type[scalar_count];
        alignas(align_bound) vector_type m_value;
    ...
    public:
        ...
        explicit simd_auto(Args... args) : m_value{static_cast<scalar_type>(args)...}
        {
        }

        constexpr       scalar_type *data()       { return m_value; }
        constexpr const scalar_type *data() const { return m_value; }
        ...
        static constexpr simd_auto zeros() { return simd_auto(scalar_type(0)); }
        static constexpr simd_auto ones () { return simd_auto(scalar_type(1)); }
