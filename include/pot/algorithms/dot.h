...
[[nodiscard]] pot::coroutines::lazy_task<T> dot_simd(pot::executor &exec, std::span<const T> a,
                                                     std::span<const T> b)
 ...
 */
template <typename T, pot::simd::SIMDType ST>
[[nodiscard]] pot::coroutines::lazy_task<T> dot_simd(pot::executor &exec, const std::vector<T> &a,
 ...
 */
template <typename T>
[[nodiscard]] pot::coroutines::lazy_task<T> dot(pot::executor &exec, std::span<const T> a,
                                                std::span<const T> b)
    requires(std::is_arithmetic_v<T>)
{
    if (a.size() != b.size())
 ...
 */
template <typename T>
[[nodiscard]] pot::coroutines::lazy_task<T> dot(pot::executor &exec, const std::vector<T> &a,
                                                const std::vector<T> &b)
    requires(std::is_arithmetic_v<T>)
{
    if (a.size() != b.size())
